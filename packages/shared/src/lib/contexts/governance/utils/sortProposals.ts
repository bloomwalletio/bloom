...
export function sortProposals(proposals: IProposalWithStatus[], filter: IProposalFilter): IProposalWithStatus[] {
...
function sortByPhaseAndMilestoneAndName(
    proposal1: IProposalWithStatus,
    proposal2: IProposalWithStatus,
    asc: boolean
): number {
    return (
...
function sortByName(proposal1: IProposalWithStatus, proposal2: IProposalWithStatus, asc: boolean): number {
    return proposal1.title.toLowerCase() > proposal2.title.toLowerCase() ? (asc ? 1 : -1) : asc ? -1 : 1
}

function sortByPhase(proposal1: IProposalWithStatus, proposal2: IProposalWithStatus, asc: boolean): number {
...
function sortByMilestone(proposal1: IProposalWithStatus, proposal2: IProposalWithStatus, asc: boolean): number {
