...
export async function generateActivityFromEvmTransaction(
    ...
    if (asset?.type === AssetType.Token || asset?.type === AssetType.BaseCoin) {
        return generateTokenActivity(transaction, networkId, chain, asset.tokenId, asset.rawAmount, recipientAddress)
    } else if (asset?.type === AssetType.Nft) {
        return generateNftActivity(transaction, networkId, chain, asset.nftId, recipientAddress)
    }
}
