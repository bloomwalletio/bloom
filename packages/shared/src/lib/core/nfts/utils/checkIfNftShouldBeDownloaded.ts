...
export async function checkIfNftShouldBeDownloaded(
    ...
    try {
        ...
        if (alreadyDownloaded) {
            return { shouldDownload: false, downloadMetadata, isLoaded: true }
            ...
            if (validation?.error || validation?.warning) {
                ...
                if (features.collectibles.useCaching.enabled) {
                    return { shouldDownload: true, downloadMetadata, isLoaded: false }
                } else {
                    return { shouldDownload: false, downloadMetadata, isLoaded: true }
                }
            }
...
async function getNftDownloadData(nft: INft): Promise<Partial<DownloadMetadata>> {
    ...
    if (persistedNftData && persistedNftData.downloadMetadata.error?.message !== UNREACHABLE_ERROR_MESSAGE) {
