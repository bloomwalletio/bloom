...
export async function isErc721NftSpendable(nft: IPersistedErc721Nft | IErc721Nft): Promise<boolean> {
    const { networkId, contractMetadata, tokenId, ownerAddress } = nft
    ...
    return owner.toLowerCase() === ownerAddress
}
