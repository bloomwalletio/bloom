...
describe('formatTokenAmountBestMatch', () => {
    ...
    test('should handle bigint amount with tokenMetadata but without decimals', () => {
        ...
        expect(formatTokenAmountBestMatch(1000n, mockTokenMetadata)).toEqual('1,000 BTC')
    })

    test('should handle rounding with small decimal amount', () => {
        const mockTokenMetadata = { decimals: 2 } as TokenMetadata
        expect(formatTokenAmountBestMatch(999n, mockTokenMetadata)).toEqual('9.99 BTC')
    })

    test('should handle rounding with hig decimal amount', () => {
        const mockTokenMetadata = { decimals: 2 } as TokenMetadata
        expect(formatTokenAmountBestMatch(1n, mockTokenMetadata)).toEqual('0.01 BTC')
    })

    test('should handle no rounding with integer amount', () => {
        const mockTokenMetadata = { decimals: 0 } as TokenMetadata
        expect(formatTokenAmountBestMatch(1234n, mockTokenMetadata, { round: false })).toEqual('1,234 BTC')
    })

    test('should format amount correctly with large number of decimals', () => {
        const mockTokenMetadata = { decimals: 18 } as TokenMetadata
        expect(formatTokenAmountBestMatch(123006789123456789n, mockTokenMetadata)).toEqual('0.123006 BTC')
    })

    test('should return approximate symbol for very small decimals', () => {
        const mockTokenMetadata = { decimals: 18 } as TokenMetadata
        expect(formatTokenAmountBestMatch(1n, mockTokenMetadata)).toEqual('≈0 BTC')
    })

    test('should handle amount with unit and without rounding', () => {
        const mockTokenMetadata = { decimals: 4 } as TokenMetadata
        const amount = 123456789n
        expect(formatTokenAmountBestMatch(amount, mockTokenMetadata, { round: false })).toBe('12,345.6789 BTC')
    })

    test('should handle amount without unit and with rounding', () => {
        const mockTokenMetadata = { decimals: 4 } as TokenMetadata
        const amount = 123456789n
        expect(formatTokenAmountBestMatch(amount, mockTokenMetadata, { withUnit: false })).toBe('12,345.6')
    })

    test('should handle very large amounts correctly', () => {
        const mockTokenMetadata = { decimals: 2 } as TokenMetadata
        const amount = 1000000000000000000000n
        expect(formatTokenAmountBestMatch(amount, mockTokenMetadata)).toEqual('10,000,000,000,000,000,000 BTC')
    })

    test('should handle amounts with maximum decimals', () => {
        const mockTokenMetadata = { decimals: 18 } as TokenMetadata
        const amount = 123456789123456789n
        expect(formatTokenAmountBestMatch(amount, mockTokenMetadata)).toEqual('0.123456 BTC')
    })

    test('should handle zero amount', () => {
        const mockTokenMetadata = { decimals: 2 } as TokenMetadata
        expect(formatTokenAmountBestMatch(0n, mockTokenMetadata)).toEqual('0 BTC')
    })

    test('should handle zero amount without unit', () => {
        const mockTokenMetadata = { decimals: 0 } as TokenMetadata
        expect(formatTokenAmountBestMatch(0n, mockTokenMetadata, { withUnit: false })).toEqual('0')
    })

    test('should handle amount with no decimals and rounding', () => {
        const mockTokenMetadata = { decimals: 0 } as TokenMetadata
        expect(formatTokenAmountBestMatch(123456789n, mockTokenMetadata)).toEqual('123,456,789 BTC')
    })

    test('should handle edge case where decimal part is all zeros', () => {
        const mockTokenMetadata = { decimals: 5 } as TokenMetadata
        expect(formatTokenAmountBestMatch(100000n, mockTokenMetadata)).toEqual('1 BTC')
    })

    test('should handle negative numbers', () => {
        const amount = BigInt(-42069)
        const mockTokenMetadata = { decimals: 1 } as TokenMetadata
        const formattedAmount = formatTokenAmountBestMatch(amount, mockTokenMetadata, { withUnit: false })
    ...
    test('should add correct decimal character for german', () => {
        ...
        const formattedAmount = formatTokenAmountBestMatch(amount, mockTokenMetadata, { withUnit: false, round: false })
    ...
    test('should return dash for big decimal amount', () => {
        ...
        const formattedAmount = formatTokenAmountBestMatch(amount, mockTokenMetadata, { withUnit: false, round: true })
        expect(formattedAmount).toBe('-')
    })
