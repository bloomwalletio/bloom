...
describe('File: parseCurrency.ts', () => {
    ...
    it('should work with large numbers and no decimals', () => {
        const amount = '20,999,999.9999999991108075'
        const parsedAmount = parseCurrency(amount)
        expect(parsedAmount).toBe('20999999.9999999991108075')
    })
    ...
    it('should work with a decimal number without a leading 0', () => {
        const amount = '.'
        const parsedAmount = Number(parseCurrency(amount))
